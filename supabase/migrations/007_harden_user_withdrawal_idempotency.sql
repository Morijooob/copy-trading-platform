...
begin
  ...
  insert into public.wallet_withdrawal_requests(
    user_id,currency,amount,destination,status,idempotency_key,metadata
  ) values (
    p_user_id,p_currency,p_amount,trim(p_destination),'pending',
    nullif(trim(p_idempotency_key),''),
    jsonb_build_object('execution_locked',true)
