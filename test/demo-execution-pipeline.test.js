...

{
  ...
  assert.equal(result.order.filledQty, 5);
  assert.equal(risk.exposure, 500);
  assert.equal(risk.reservedExposure, 0);
}

{
  const { pipeline } = makePipeline({ scenarios: { partial: "PARTIAL_FILL" } });
...

{
  ...
  assert.equal(recovered.order.filledQty, 0);
...

{
  ...
  assert.equal(recovered.status, "CONFIRMED");
  assert.equal(recovered.order.status, "FILLED");
...

{
  ...
  const first = risk.reserve({ side: "BUY", quantity: 6, price: 100 });
  assert.equal(first.approved, true);
  const second = risk.reserve({ side: "BUY", quantity: 5, price: 100 });
  ...
  risk.releaseReservation(first.reservationId);
  assert.equal(risk.reservedExposure, 0);
