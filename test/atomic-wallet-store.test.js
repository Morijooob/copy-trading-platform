...
test('atomic wallet transaction serializes concurrent mutations', async () => {
  const ledger = new WalletLedger(); credit(ledger, 1000); const saves = [];
  const store = new AtomicWalletStore({ ledger, stateStore: { save: async (snapshot) => saves.push(snapshot) } });
  await Promise.all([store.transact(async (wallet) => { await new Promise(r => setTimeout(r, 5)); debit(wallet, 300); }), store.transact(async (wallet) => debit(wallet, 400))]);
  assert.equal(ledger.balance('user-A', 'USDT'), 300); assert.equal(saves.length, 2);
});

test('failed durable save rolls the wallet back', async () => {
  const ledger = new WalletLedger(); credit(ledger, 1000);
...
test('queue continues after a failed transaction', async () => {
  const ledger = new WalletLedger(); credit(ledger, 1000); let first = true;
  const store = new AtomicWalletStore({ ledger, stateStore: { save: async () => { if (first) { first = false; throw new Error('temporary failure'); } } } });
  await assert.rejects(store.transact(async (wallet) => debit(wallet, 100))); await store.transact(async (wallet) => debit(wallet, 200));
