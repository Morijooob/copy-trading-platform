...
function makeAccount(accountId, maxExposure = 100000, maxOrderNotional = 100000, maxDailyLoss = 100000) {
  return {
    accountId,
    riskEngine: new RiskEngine({ maxOrderNotional, maxDailyLoss, maxExposure }),
...
test('multi-account API stress: isolation, many orders, replay, partial/full fill', async () => {
  ...
  for (const accountId of ['A', 'B', 'C']) {
    for (let i = 0; i < 40; i += 1) placed.push(await api.placeOrder({ token: tokens[accountId], symbol: i % 2 ? 'BTCUSDT' : 'ETHUSDT', side: i % 3 === 0 ? 'SELL' : 'BUY', quantity: 1 + i / 100, price: 100 + i, clientOrderId: `stress:${accountId}:${i}` }));
  }
  ...
  await assert.rejects(() => api.getOrder({ token: tokens.A, clientOrderId: 'stress:B:0' }), /order not found|forbidden/i);
  await assert.rejects(() => api.getOrder({ token: tokens.B, clientOrderId: 'stress:A:0' }), /order not found|forbidden/i);
});
