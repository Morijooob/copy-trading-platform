...

{
  ...
  assert.equal(submitted.status, "UNKNOWN"); assert.equal(first.exchange.getAuditLog().filter((e) => e.type === "ORDER_ACCEPTED").length, 1);
  ...
  assert.equal(recovered[0].status, "CONFIRMED"); assert.equal(recovered[0].order.status, "PENDING"); assert.equal(second.risk.exposure, 0); assert.equal(second.risk.reservedExposure, 200);
  const replay = second.pipeline.submit({ symbol: "BTCUSDT", side: "BUY", quantity: 2, price: 100, clientOrderId: "restart-crash" });
  assert.equal(replay.order.exchangeOrderId, recovered[0].order.exchangeOrderId); assert.equal(second.exchange.getAuditLog().filter((e) => e.type === "ORDER_ACCEPTED").length, 1);
}

{
  const first = makeComponents({ scenarios: { "restart-timeout": "TIMEOUT" } });
  first.pipeline.submit({ symbol: "BTCUSDT", side: "BUY", quantity: 10, price: 100, clientOrderId: "restart-timeout" });
  const state = first.pipeline.exportState();
  const second = makeComponents({ exchangeState: first.exchange.exportState(), riskState: state.risk, executionState: state.execution, pipelineState: state });
  const recovered = second.pipeline.recoverAfterRestart();
  assert.equal(recovered[0].status, "CONFIRMED"); assert.equal(recovered[0].order.filledQty, 0); assert.equal(second.risk.exposure, 0); assert.equal(second.risk.reservedExposure, 1000);
}

{
  const first = makeComponents(); first.exchange.disconnect();
  first.pipeline.submit({ symbol: "BTCUSDT", side: "BUY", quantity: 3, price: 100, clientOrderId: "restart-disconnect" });
  const state = first.pipeline.exportState(); first.exchange.reconnect();
  const second = makeComponents({ exchangeState: first.exchange.exportState(), riskState: state.risk, executionState: state.execution, pipelineState: state });
  assert.equal(second.pipeline.recoverAfterRestart()[0].status, "UNKNOWN"); assert.equal(second.risk.reservedExposure, 300);
  const retried = second.pipeline.recover("restart-disconnect");
  assert.equal(retried.status, "CONFIRMED"); assert.equal(retried.order.status, "FILLED"); assert.equal(second.risk.exposure, 300); assert.equal(second.risk.reservedExposure, 0); assert.equal(second.exchange.getAuditLog().filter((e) => e.type === "ORDER_ACCEPTED").length, 1);
}

{
  const a = makeComponents({ scenarios: { f1: "CRASH" } }); const b = makeComponents();
  ...
  assert.equal(r1.status, "UNKNOWN"); assert.equal(r2.status, "CONFIRMED"); assert.equal(b.risk.exposure, 100);
  const stateA = a.pipeline.exportState(); const stateB = b.pipeline.exportState();
  ...
  assert.equal(ar.pipeline.recoverAfterRestart()[0].order.status, "PENDING"); assert.equal(ar.risk.exposure, 0); assert.equal(ar.risk.reservedExposure, 200);
  assert.equal(br.pipeline.recoverAfterRestart().length, 0); assert.equal(br.risk.exposure, 100); assert.equal(br.risk.reservedExposure, 0);
  assert.equal(ar.exchange.getAuditLog().filter((e) => e.type === "ORDER_ACCEPTED").length, 1); assert.equal(br.exchange.getAuditLog().filter((e) => e.type === "ORDER_ACCEPTED").length, 1);
}

{
  ...
  const second = makeComponents({ exchangeState: first.exchange.exportState(), riskState: state.risk, executionState: state.execution, pipelineState: state });
  const recovered = second.pipeline.recoverAfterRestart();
  assert.equal(recovered[0].order.status, "PARTIAL"); assert.equal(recovered[0].order.filledQty, 5); assert.equal(second.risk.exposure, 500); assert.equal(second.risk.reservedExposure, 500);
...
{
  const first = makeComponents(); first.exchange.disconnect(); first.pipeline.submit({ symbol: "BTCUSDT", side: "BUY", quantity: 2, price: 100, clientOrderId: "tamper" });
  const state = first.pipeline.exportState(); state.risk.reservedExposure = 0;
