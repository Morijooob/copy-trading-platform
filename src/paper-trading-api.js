...
export class PaperTradingApi {
  ...
  placeOrder({ token, symbol, side, quantity, price, clientOrderId }) {
    ...
    return { accountId: session.accountId, symbol, side, quantity: Number(quantity), ...result };
  }
  getOrder({ token, clientOrderId }) {
    const session = this.authenticate(token);
    const record = this.paperExchange.orders.get(clientOrderId);
    if (!record || record.request.accountId !== session.accountId) throw new Error('order not found');
    return { accountId: session.accountId, symbol: record.request.symbol, side: record.request.side, quantity: record.request.quantity, ...this.paperExchange.snapshot(record) };
  }
  listOrders({ token }) {
    const session = this.authenticate(token);
    return Array.from(this.paperExchange.orders.values()).filter((o) => o.request.accountId === session.accountId).map((o) => ({ accountId: session.accountId, symbol: o.request.symbol, side: o.request.side, quantity: o.request.quantity, ...this.paperExchange.snapshot(o) }));
  }
  dashboard({ token }) {
    const session = this.authenticate(token);
