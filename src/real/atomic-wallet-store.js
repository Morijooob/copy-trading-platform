export class AtomicWalletStore {
  ...
  constructor({ ledger, stateStore }) {
    if (!ledger || !stateStore || typeof stateStore.save !== 'function') throw new Error('ledger and durable stateStore are required');
    this.#ledger = ledger;
  ...
  #enqueue(operation) { const run = this.#lock.then(operation, operation); this.#lock = run.catch(() => undefined); return run; }
  #restoreLedger(snapshot) {
    if (typeof this.#ledger.restore !== 'function') throw new Error('ledger restore capability is required for atomic rollback');
