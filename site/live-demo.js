(() => {
  ...
  class Engine {
    ...
    reset(capital = this.capital) { this.capital = Math.max(1, Number(capital) || 100); this.cash=this.capital; this.position=null; this.realized=0; this.fees=0; this.orders=0; this.cycleCount=0; this.cooldown=0; this.lastPrices={}; this.lastSignals={}; this.events=[]; }
    ...
    process(markets) {
      ...
      const ranked=Object.entries(signals).filter(([,s])=>s.score>=CONFIG.minScore).sort((a,b)=>b[1].score-a[1].score); const best=ranked[0]?{symbol:ranked[0][0],...ranked[0][1]}:null;
      let closed=false;
      if(this.position){ this.position.holdCycles++; const s=signals[this.position.symbol], entry=this.position.entry, price=this.lastPrices[this.position.symbol]; const move=this.position.side==='LONG'?(price-entry)/entry:(entry-price)/entry; let reason=null; if(move>=CONFIG.takeProfitPct)reason='take-profit'; else if(move<=-CONFIG.stopLossPct)reason='stop-loss'; else if(this.position.holdCycles>=CONFIG.maxHoldCycles)reason='max-hold'; else if(s&&((this.position.side==='LONG'&&s.signal==='SELL')||(this.position.side==='SHORT'&&s.signal==='BUY')))reason='reverse-signal'; if(reason)closed=!!this.close(reason); }
  ...
  function render(s){
    ...
    $('equity').textContent=fmt(s.equity); $('unrealized').textContent=fmt(s.unrealized); $('realized').textContent=fmt(s.realized); $('fees').textContent=fmt(s.fees); $('total').textContent=fmt(s.totalPnl); $('orders').textContent=s.orders; $('cycles').textContent=s.cycleCount; $('position').textContent=s.position?`${s.position.side} · ${s.position.symbol}`:'FLAT'; $('signal').textContent=s.best?(s.best.signal==='BUY'?'LONG':'SHORT'):'NO TRADE'; $('score').textContent=s.best?s.best.score:'—'; $('symbol').textContent=s.best?s.best.symbol:'—'; $('btc').textContent=fmt(s.lastPrices.BTC); $('eth').textContent=fmt(s.lastPrices.ETH); $('sol').textContent=fmt(s.lastPrices.SOL); $('updated').textContent=lastUpdate?new Date(lastUpdate).toLocaleTimeString('fa-IR'):'—';
